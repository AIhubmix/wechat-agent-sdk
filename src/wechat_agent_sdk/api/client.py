...
class ILinkBotClient:
    ...
    async def get_updates(self, cursor: str = "") -> tuple[list[dict], str]:
        ...
        new_cursor = data.get("get_updates_buf") or data.get("next_cursor") or cursor
        msgs = data.get("msgs") or data.get("updates") or data.get("messages") or []
    ...
    async def check_login_status(self, qr_uuid: str) -> dict:
        ...
        Returns {"status": "pending|scanned|confirmed|expired|error", "token"?: str}
        """
        client = await self._ensure_client()
        ...
        status = data.get("status") or data.get("wx_status") or ""

        # Check if confirmed with token
        if data.get("ret") == 0:
            token = (
                data.get("token")
                or data.get("ilink_bot_token")
        ...
        if status_lower in ("confirmed", "authorized", "login", "logged_in"):
            token = (
                data.get("token")
                or data.get("ilink_bot_token")
                or data.get("bot_token")
        ...
        elif status_lower in ("scanned", "scan"):
